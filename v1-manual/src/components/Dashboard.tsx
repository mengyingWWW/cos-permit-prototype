...
import { TrendingUp, TrendingDown, Info, ChevronDown, CheckCircle2, Circle, Clock, ExternalLink } from 'lucide-react';
...
import {
  ...
  BAR_CHART_DATA,
  LINE_CHART_DATA,
  PARTIAL_PERMIT_CLASSES,
  PARTIAL_PERMIT_TYPES,
  PARTIAL_CASE_TIMELINES,
  PARTIAL_TIMELINE_STAGES_TEMPLATE,
  MANUAL_PROCESSING_TIMES,
} from '../data';
...
// ── Partial: case-based tracker ───────────────────────────────────────────────

function CaseBasedTracker() {
  const [permitClass, setPermitClass] = useState('');
  const [permitType, setPermitType] = useState('');
  const [zipCode, setZipCode] = useState('');

  const availableTypes = permitClass ? PARTIAL_PERMIT_TYPES[permitClass] ?? [] : [];
  const caseData = permitType ? PARTIAL_CASE_TIMELINES[permitType] : null;

  const buildPartialStages = (planDays: number, corrDays: number) => {
    return PARTIAL_TIMELINE_STAGES_TEMPLATE.map((s, idx) => {
      const days = s.id === 'plan_review' ? planDays : s.id === 'correction' ? corrDays : s.estimatedDays;
      return {
        ...s,
        estimatedDays: days,
        startDate: '—',
        endDate: '—',
        status: (idx < 2 ? 'upcoming' : idx === 2 ? 'active' : 'upcoming') as 'completed' | 'active' | 'upcoming',
      };
    });
  };
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-base font-semibold text-gray-900">
          Typical Permit Timeline Estimate
        </h2>
        <div className="flex items-center gap-1.5 mt-0.5">
          <div className="w-3.5 h-3.5 rounded-full bg-ai-500 flex items-center justify-center">
            <span className="text-white text-[8px] font-bold">AI</span>
          ...
          <p className="text-xs text-gray-500">
            Case-based estimate — select a permit type to see typical timelines from similar past applications.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative">
          <select
            value={permitClass}
            onChange={e => { setPermitClass(e.target.value); setPermitType(''); }}
            className="appearance-none pl-3 pr-8 py-2 text-xs bg-white border border-gray-200 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-seattle-500 cursor-pointer min-w-[160px]"
          >
            <option value="">Permit Class</option>
            {PARTIAL_PERMIT_CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        ...
        <div className="relative">
          <select
            ...
            disabled={!permitClass}
            className="appearance-none pl-3 pr-8 py-2 text-xs bg-white border border-gray-200 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-seattle-500 cursor-pointer min-w-[160px] disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <input
          ...
          maxLength={5}
          className="pl-3 pr-3 py-2 text-xs bg-white border border-gray-200 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-seattle-500 w-28"
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white border border-gray-200 rounded-xl px-5 py-4 shadow-sm">
          ...
          <p className={`text-3xl font-bold mt-1 ${caseData ? 'text-gray-900' : 'text-gray-400'}`}>
            {caseData ? caseData.planReview : '--'}{' '}
            <span className={`text-lg font-semibold ${caseData ? 'text-gray-500' : 'text-gray-300'}`}>Days</span>
          </p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl px-5 py-4 shadow-sm">
          <div className="flex items-center justify-between mb-1">
      ...
      {/* Basis note */}
      {caseData && (
        <div className="flex items-start gap-2 bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 fade-in">
          <Info size={13} className="text-gray-400 flex-shrink-0 mt-0.5" />
          <p className="text-xs text-gray-500">
            Median across <span className="font-semibold text-gray-700">{caseData.sampleSize} similar {permitType} applications</span> filed in Seattle over the past 24 months. This is a typical-case estimate, not personalized to your project.
          </p>
        </div>
      )}

      {/* Timeline stages (only when permit type selected) */}
      {caseData && (
        <div className="bg-white border border-gray-200 rounded-xl px-5 py-4 shadow-sm fade-in">
          <div className="flex items-center gap-2 mb-4">
            <Clock size={14} className="text-gray-500" />
            <h3 className="text-sm font-semibold text-gray-700">Typical Review Stages</h3>
          </div>
          <div className="relative">
            {buildPartialStages(caseData.planReview, caseData.correction).map((stage, idx, arr) => (
              <div key={stage.id} className="flex gap-4 relative">
                {idx < arr.length - 1 && (
                  <div className="absolute left-[11px] w-0.5 bg-gray-200 z-0" style={{ top: 22, bottom: -8 }} />
                )}
                <div className="relative z-10 flex-shrink-0 mt-1">
                  <Circle size={22} className="text-gray-300 bg-white" />
                </div>
                <div className={`flex-1 pb-5 ${idx === arr.length - 1 ? 'pb-0' : ''}`}>
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="text-sm font-medium text-gray-600">{stage.label}</span>
                    <span className="text-xs flex-shrink-0 text-gray-400">~{stage.estimatedDays} days (typical)</span>
                  </div>
                </div>
              </div>
            ))}
...
// ── Manual: static processing times reference ─────────────────────────────────

function ProcessingTimesTable() {
  const [expanded, setExpanded] = useState<string | null>('Roofing & Envelope');

  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-base font-semibold text-gray-900">
          SDCI Standard Processing Times
        </h2>
        <p className="text-xs text-gray-500 mt-0.5">
          Published reference times by permit type. Use these to estimate your own schedule.
          Actual timelines vary based on application completeness and current review queue.
        </p>
      </div>
      ...
      <div className="flex items-start gap-2.5 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3">
        ...
        <div>
          <p className="text-xs text-blue-800 font-medium">How to use this table</p>
          <p className="text-xs text-blue-700 mt-0.5 leading-relaxed">
            Find your permit type below to see typical plan review, correction, and total issuance windows.
            All times are in <strong>calendar days</strong> from application submission.
            For current queue times, contact SDCI at <span className="font-mono">(206) 684-8850</span>.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {MANUAL_PROCESSING_TIMES.map(group => (
          <div key={group.category} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
            <button
            ...
            >
              ...
              <ChevronDown
                size={15}
                className={`text-gray-400 transition-transform ${expanded === group.category ? 'rotate-180' : ''}`}
              />
            </button>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      <th className="text-left px-5 py-2 font-semibold text-gray-500 uppercase tracking-wide text-[10px]">Permit Type</th>
                      <th className="text-center px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide text-[10px]">Plan Review (days)</th>
                      <th className="text-center px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide text-[10px]">Corrections (days)</th>
                      <th className="text-center px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide text-[10px]">Total Issuance (days)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {group.permits.map(p => (
                      <tr key={p.type} className="hover:bg-gray-50 transition-colors">
                        <td className="px-5 py-2.5 text-gray-800 font-medium">{p.type}</td>
                        <td className="px-3 py-2.5 text-center text-gray-600 font-mono">{p.planReview}</td>
                        <td className="px-3 py-2.5 text-center text-gray-600 font-mono">{p.correction}</td>
                        <td className="px-3 py-2.5 text-center text-gray-700 font-semibold font-mono">{p.totalIssue}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className="flex items-center gap-1.5 text-xs text-gray-500">
        ...
        <span>Source: Seattle Department of Construction & Inspections (SDCI) — sdci.seattle.gov</span>
      </div>
    </div>
  );
}

// ── End manual reference table ────────────────────────────────────────────────
    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      <div className="flex flex-1 gap-0 min-h-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin px-7 py-6">
          ...
          ) : AI_CONFIG.timeline === 'partial' ? (
            <CaseBasedTracker />
          ) : AI_CONFIG.timeline === 'manual' ? (
