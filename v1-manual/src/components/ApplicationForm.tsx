...
export default function ApplicationForm({ onNavigate }: ApplicationFormProps) {
  ...
  const [docs, setDocs] = useState(
    AI_CONFIG.formFill === 'partial' ? PARTIAL_DOCUMENTS : INITIAL_DOCUMENTS
  );
        ...
        <div className="max-w-[1100px] mx-auto px-6 py-6">
          ...
          <div className="flex gap-5">
            ...
            <div className="w-[280px] min-w-[280px] flex flex-col gap-4">
              ...
              {AI_CONFIG.formFill === 'manual' && (
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm">
                  <div className="px-4 pt-4 pb-3 border-b border-gray-100">
                    <h3 className="text-sm font-semibold text-gray-700">Quick Actions</h3>
                  </div>
                  <div className="px-4 py-3 space-y-2">
                    {['Save as Draft', 'Clear Form', 'Import from Existing'].map(action => (
                      <button
                        key={action}
                        onClick={() => action === 'Clear Form' && setFormData(INITIAL_FORM_DATA)}
                        className="w-full text-left px-4 py-2.5 text-sm text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-gray-300 transition-colors"
                      >
                        {action}
                      </button>
                    ))}
